	...
	for pi := range points {
		...
		for j := range neighbourIds {
			...
			if zoom < b.zoom {
				...
				nPoints += b.NumPoints
				neighbours = append(neighbours, b.IncludedPoints...)
		...
		if len(foundNeighbours) > 0 {
			...
			newCluster.IncludedPoints = neighbours
...
func translateGeoPointsToClusterPoints(points []GeoPoint) []*ClusterPoint {
	...
	for i, p := range points {
		...
		cp.X, cp.Y = MercatorProjection(coordinates)
		result[i] = &cp
		cp.NumPoints = 1
		cp.IncludedPoints = []GeoPoint{p}
		cp.Id = i
	}
	return result
}
